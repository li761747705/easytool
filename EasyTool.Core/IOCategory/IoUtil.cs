...
namespace EasyTool
{
    ...
    public static class IoUtil
    {
        /// <summary>
        /// 读取文件的所有行到一个字符串数组中
        /// [Obsolete("请直接使用 File.ReadAllLines(path)")]
        /// </summary>
        /// <param name="path">文件路径</param>
        /// <returns>字符串数组，其中包含文件的所有行。</returns>
        [Obsolete("请直接使用 File.ReadAllLines(path)", false)]
        ...
        /// <summary>
        /// 将字符串数组写入文件，覆盖原有内容
        /// [Obsolete("请直接使用 File.WriteAllLines(path, lines)")]
        /// </summary>
        /// <param name="path">文件路径</param>
        /// <param name="lines">待写入的字符串数组</param>
        [Obsolete("请直接使用 File.WriteAllLines(path, lines)", false)]
        ...
        [Obsolete("请直接使用 File.ReadAllText(path)", false)]
        ...
        /// <summary>
        /// 将字符串写入文件，覆盖原有内容
        /// [Obsolete("请直接使用 File.WriteAllText(path, text)")]
        /// </summary>
        /// <param name="path">文件路径</param>
        /// <param name="text">待写入的字符串</param>
        [Obsolete("请直接使用 File.WriteAllText(path, text)", false)]
        ...
        /// <summary>
        /// 读取二进制数据到一个字节数组中
        /// [Obsolete("请直接使用 File.ReadAllBytes(path)")]
        /// </summary>
        /// <param name="path">文件路径</param>
        /// <returns></returns>
        [Obsolete("请直接使用 File.ReadAllBytes(path)", false)]
        ...
        /// <summary>
        /// 将字节数组写入二进制文件，覆盖原有内容
        /// [Obsolete("请直接使用 File.WriteAllBytes(path, bytes)")]
        /// </summary>
        /// <param name="path">文件路径</param>
        /// <param name="bytes">待写入的字节数组</param>
        [Obsolete("请直接使用 File.WriteAllBytes(path, bytes)", false)]
        ...
        /// <summary>
        /// 将一个字符串转换为字节数组
        /// [Obsolete("请直接使用 Encoding.UTF8.GetBytes(text)")]
        /// </summary>
        /// <param name="text">待转换的字符串</param>
        /// <returns>字节数组，其中包含输入字符串的编码数据</returns>
        [Obsolete("请直接使用 Encoding.UTF8.GetBytes(text)", false)]
